...
function Index() {
  ...
  const joinTable = async (e: React.FormEvent) => {
    ...
    setJoinError(null);
    try {
      const data = await joinTableAction({ 
        data: { 
          tableId, 
    ...
    return (
      <main className="stage-bg min-h-screen font-body text-white/90">
        <div className="mx-auto flex max-w-2xl flex-col items-center gap-6 px-4 py-20">
          ...
          <DiscordAuth onAuthChange={setAuthState} />
          ...
          <div className="text-right space-y-1">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-full border border-gold/40 bg-gold/10 px-3 py-1 text-xs font-display tracking-wider text-gold uppercase hover:bg-gold/20 transition-colors"
              >
                🏆 Leaderboard
              </a>
            </div>
